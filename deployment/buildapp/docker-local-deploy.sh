...
docker build -t ${DOCKER_TAG} --build-arg jar_name=${JAR_FILE} \
--build-arg git_commit=${GIT_COMMIT_LABEL} --build-arg build_number=${BUILD_NUMBER} -f ./deployment/buildapp/Dockerfile .
